...
uint16_t        char_mapping[OLED_MATRIX_SIZE / (OLED_DISPLAY_HEIGHT / OLED_FONT_HEIGHT) / OLED_FONT_WIDTH * (OLED_DISPLAY_HEIGHT / OLED_FONT_HEIGHT)];
uint8_t         char_map_index      = 0;
...
#endif
    max_chars = OLED_DISPLAY_HEIGHT / OLED_FONT_HEIGHT;
    max_lines = OLED_DISPLAY_WIDTH / OLED_FONT_WIDTH; 

    char_map_index = OLED_MATRIX_SIZE / (OLED_DISPLAY_HEIGHT / OLED_FONT_HEIGHT) / OLED_FONT_WIDTH * (OLED_DISPLAY_HEIGHT / OLED_FONT_HEIGHT);
    for (uint8_t i = 0; i < char_map_index; i++) {
      uint8_t col = i % max_chars;
      uint8_t row = i / max_chars;
      char_mapping[i] = ((col + 1) * OLED_DISPLAY_WIDTH) - ((row + 1) * OLED_FONT_WIDTH);
    }
...
void oled_clear(void) {
    ...
    oled_cursor = &oled_buffer[char_mapping[0]];
...
void oled_set_cursor(uint8_t col, uint8_t line) {
    uint16_t index = col + (line * max_chars);

    // Out of bounds?
    if (index >= char_map_index) {
        index = 0;
    }
    current_char_index = index;
    oled_cursor = &oled_buffer[char_mapping[index]];
}

void oled_advance_page(bool clearPageRemainder) {
    uint8_t col = current_char_index % max_chars;
    if (col == 0) return;
    uint8_t  remaining = max_chars - col;
    if (clearPageRemainder) {
        // Write empty character until next line
        while (remaining--)
            oled_write_char(' ', false);
    } else {
        current_char_index = current_char_index / max_chars * max_chars + max_chars;
        // Next page index out of bounds?
        if (current_char_index >= char_map_index) {
            current_char_index     = 0;
        }

        oled_cursor = &oled_buffer[char_mapping[current_char_index]];
    }
}
...
__attribute__((weak)) void oled_task_user_once(void) {
}
